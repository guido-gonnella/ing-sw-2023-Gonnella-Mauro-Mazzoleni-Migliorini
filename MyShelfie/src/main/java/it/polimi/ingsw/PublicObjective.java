...
CommonObj angles = (shelf) ->{
    ...
    if(temp[0][0].isPresent() && temp[6][0].isPresent() && temp[0][5].isPresent() && temp[6][5].isPresent()){
        if(temp[0][0].get().getType().equal(temp[6][0].get().getType()) &&
        temp[6][0].get().getType().equal(temp[0][5].get().getType()) &&
        temp[0][5].get().getType().equal(temp[6][5].get().getType())) return true;
    }
    return false;
};
    ...
    for(int i = 0; i < 5; i++){
        for(int j = 0; j < 4; j++){
            if(temp[i][j].isPresent() && temp[i+1][j].isPresent() && temp[i][j+1].isPresent() && temp[i+1][j+1].isPresent()){
                if(temp[i][j].get().getType().equal(temp[i+1][j].get().getType()) &&
                temp[i+1][j].get().getType().equal(temp[i+1][j+1].get().getType()) &&
                temp[i+1][j+1].get().getType().equal(temp[i][j+1].get().getType()) &&
...
CommonObj FourFourEqual = (shelf) -> {
    ...
    for(int i = 0; i < 6; i++){
        for(int j = 0; j < 5; j++){
            if(!check[i][j] && temp.get()[i][j].isPresent()){
                ...
                while(!q.isEmpty() && nEqualTiles < 4){
                    ...
                    if((temp[x+1][y].isEmpty() || temp[x+1][j].get().getType().equal(temp[x][y].get().getType()) && !check[x+1][y] && x+1 >= 0 && x+1 < 6){
                    ...
                    if((temp[x][y+1].isEmpty() || temp[x][j+1].get().getType().equal(temp[x][y].get().getType()) && !check[x][y+1] && y+1 >= 0 && y+1 < 5){
        ...
        for(int j = 0; j < 5 && flag; j++){
            if(temp[i][j].isEmpty()){
                ...
                switch(temp[i][j].get().getType()){
                    case Type.CAT: nTs[j][0]++;
                    case Type.GAME: nTs[j][1]++;
                    case Type.FRAME: nTs[j][2]++;
                    case Type.PLANT: nTs[j][3]++;
                    case Type.TROPHY: nTs[j][4]++;
                    case Type.BOOK: nTs[j][5]++;
                }
            }
        }
    }
